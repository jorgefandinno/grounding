...
{\color{comment}\% prepared Component\Sub{1,1}}
omit(X,Y) :- \alert{n(edge(X,Y))}, not path(X,Y).
{\color{comment}\% prepared Component\Sub{1,2}}
path(X,Y) :- \alert{n(edge(X,Y))}, not omit(X,Y).
{\color{comment}\% prepared Component\Sub{2,1}}
:- \alert{n(path(X,Y))}, \alert{n(path(X',Y))}, X < X'.
...
{\color{comment}\% prepared Component\Sub{7,1}}
    ...
    \Fn{\(\GroundSemiNaive(P,A)\)}{
      ...
      \ForEach{\(C\) \In \(L_P\)}{
        ...
        \Repeat{\(N=\emptyset\)}{
          ...
          \(G' \leftarrow \alert{\Ground_{O_o \cup N_n \cup A_a}(\text{prepared } C)}\)\;
...
\begin{frame}[fragile]{Example: Nonlinear Programs}
\vfill
\begin{SemiVerbatim}{.9}
trans(U,V) :- edge(U,V).

...
\begin{frame}{Semi-naive grounding}
  ...
  \item<1-> \structure{Idea} \ (originates from database systems~\cite{abhuvi95a})
    \par\smallskip
    Semi-naive grounding focuses on the new generated atoms
    \par
    at each level in order to avoid recomputing the same atoms


  \item<1->[] Inferring a new atom at step $i$ relies on at least one atom
    \par

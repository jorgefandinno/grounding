\begin{frame}{Bottom Up Grounding}
  \begin{itemize}
    ...
    \item \(\Ground_{D}(P) = \!\begin{aligned}[t] \{ r \in \Ground(P) \mid {} & \alert{\pbody{r} \subseteq D},\\ & \mbox{all comparison literals}\\ & \mbox{in \(\Body(r)\) are satisfied} \} \end{aligned}\)
  \end{itemize}
  \EmphAlgo{0.8}{
    \Fn{\(\GroundBottomUp(P,D)\)}{
      \(G \leftarrow \Ground_{D}(P)\)\;
      \If{\(\Head(G) \not\subseteq D\)}{
